...
$pmpro_groups = pmpro_get_level_groups();

$incoming_levels = pmpro_getMembershipLevelsForUser();

$displayorder = pmpro_get_levels_and_groups_in_order();
...
<div id="pmpro_levels">
	<div id="pmpro_groups">
		...
		foreach ( $displayorder as $group => $grouplevels ) {
			...
			?>
			<div id="pmpro_group-<?php echo $pmpro_groups[ $group ]->id; ?>"
			     class="pmpro_group <?php if ( intval( $pmpro_groups[ $group ]->allow_multiple_selections ) == 0 ) { ?>selectone<?php } ?>">
				<h2 class="pmpro_group-name"><?php echo $pmpro_groups[ $group ]->name ?></h2>
				<p class="pmpro_group-type">
					<?php
				...
				foreach ( $grouplevels as $level ) {
					...
					?>
					<div id="pmpro_level-<?php echo $pmpro_levels[ $level ]->id; ?>"
					     class="pmpro_level group<?php echo $group; ?> <?php if ( isset($pmpro_groups[ $group ]->allow_multiple_selections) && intval( $pmpro_groups[ $group ]->allow_multiple_selections ) == 0 ) {
						     echo 'selectone';
					     } ?>">
				...
				?>
			</div> <!-- end pmpro_group-ID -->
			<?php
			}
		}
		?>
	</div> <!-- end pmpro_groups -->
